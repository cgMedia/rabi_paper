...
Note that within this approach there is no need of an instantaneous dipole-dipole Coulomb term between the neutral dots. The interactions between the dots occur only via the field $\vb{E}(\vb{r}, t)$, which propagates between the dots with finite velocity. (See {A}$_{\text{IV}}$  and {C}$_{\text{IV}}$ of Ref.~\cite{Cohen1989} for an in-depth discussion of this point.) 
