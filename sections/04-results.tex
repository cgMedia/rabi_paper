...
\begin{table}
  ...
  \caption{\label{table:parameters}Simulation parameters (unless otherwise stated); \si{\elementarycharge} and \si{\bohr} denote the elementary charge and Bohr radius.
    The decoherence times here, while shorter than those typical of optical resonance experiments, afford a shorter computational time but realistically preserve dynamical emission phenomena.
  }
\end{table}
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}%\includegraphics[width=\columnwidth]{figures/wide_plate.png}} \\
  \caption{\label{fig:wide plate}
    Coloration of $\abs{\tilde{\rho}_{01}}$ for \num{10000} \qds{} arranged in a two-dimensional geometry (in units of $\lambda$).
    The slab displays a prominent polarization pattern  \SI{1.25}{\pico\second} after the peak of a \SI{1}{\pico\second} pulse.
  }
\end{figure}

\Cref{fig:wide plate} shows a polarization distribution for a two-dimensional ``slab'' geometry (the formulation remains three-dimensional).
Following a \SI{1}{\pico\second} pulse, the slab displays both the short-range (dark, adjacent \qds{}) and long-range (regions of enhanced/diminished polarization) phenomena observed in \cref{fig:tubes}, though the variation in these effects appears less pronounced than in \cref{fig:tubes}.
...
\begin{figure*}
  ...
  \caption{\label{fig:broadened} 
    $z$-distribution of polarization $\abs{\tilde{\rho}_{01}}$ for identical tubular geometries.
    Here, $\vu{k} \propto \vu{z}$.
    In each simulation, \num{10000} \qds{} had random Gaussian noise (of width \SIlist{0.0;0.2;0.4;0.6}{\milli\eV}) added to a resonant $\omega_0$.
    Induced patterns in the polarization remain for mild detunings $\lesssim \SI{0.5}{\milli\eV}$ but have completely washed out at \SI{0.6}{\milli\eV}.
  }
\end{figure*}
