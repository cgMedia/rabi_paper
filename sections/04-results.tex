...
\begin{table}
  ...
  \caption{\label{table:parameters}Simulation parameters; \si{\elementarycharge} and \si{\bohr} denote the elementary charge and Bohr radius.
    The decoherence times here, while shorter than those typical of optical resonance experiments, preserve dynamical emission phenomena while affording a shorter computation time.
  }
\end{table}
...
Identical simulations run without interactions (i.e.~$\mathcal{Z} = 0$ everywhere) produce homogeneous polarization distributions as the pulse varies little over the length of the cylinder.
