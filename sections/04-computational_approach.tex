...
As $\mathcal{Z}$ represents a block Toeplitz and lower triangular space/time matrix (the uniform time stepping results in its Toeplitz nature), \cref{eq:zmatrix} provides a means of calculating $\mathcal{F}^{(m)}$ given only $\mathcal{A}^{(m' \le m)}$.
...
Defining $t_n \equiv n \, \Delta t$ and approximating $\tilde{\rho}(t)$ as a weighted sum of complex exponentials, the predictor/corrector scheme proceeds with an extrapolation predictor step,
\begin{equation}
  \tilde{\rho}_{\ell}(t_{k + 1}) \leftarrow \sum_{w = 0}^{W-1} \mathcal{P}_w^{\qty(0)} \tilde{\rho}_\ell(t_{k-w}) + \mathcal{P}_w^{\qty(1)} \, \partial_t \tilde{\rho}(t_{k-w}),
  \label{eq:predictor}
\end{equation}
and several iterated corrector steps,
\begin{equation}
  \begin{aligned}
    \tilde{\rho}_\ell(t_{k + 1}) &\leftarrow \mathcal{C}_{k+1}^{\qty(1)} \, \partial_t \tilde{\rho}_\ell(t_{k+1}) \\
                                 &+ \sum_{w = 0}^{W - 1} \mathcal{C}_w^{\qty(0)} \tilde{\rho}_\ell(t_{k-w}) + \mathcal{C}_w^{\qty(1)} \, \partial_t \tilde{\rho}_\ell(t_{k-w}),
  \end{aligned}
  \label{eq:corrector}
\end{equation}
